...
卡片保留原始记录链接、历史与预测状态，以及一个结果表格按钮；正文各行连续排列。未完成原因单独一行，按平台列出，不展示备份和文件夹链接。
...
平台摘要随批次和填充状态保存，恢复任务不依赖本地临时报告。历史写入目前整体核验，任一阻断错误按降级策略处理；预测允许需要人工判断的平台保留原内容，其余平台完成后按实际结果通知。
