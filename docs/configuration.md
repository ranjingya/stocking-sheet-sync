...
兼容 `FILL_HISTORY_ENABLED` 和 `FILL_FORECAST_ENABLED` 两个老品别名；显式 `FILL_OLD_*` 优先。每次Webhook触发创建新批次并读取当前开关；同一任务恢复沿用该批次开关。
...
以下字段位于 `config.toml` 的 `[runtime]`，均为正整数：

| 字段 | 默认值 | 作用 |
| --- | --- | --- |
| `queue_max_attempts` | 3 | 后台任务最大执行次数，锁忙不计次数 |
| `queue_retry_delay_seconds` | 10 | 执行失败或锁忙后的等待秒数 |
| `queue_result_ttl_seconds` | 604800 | 已结束任务结果保留秒数，默认7天 |

排队和执行中的任务没有过期时间；业务去重记录永久保存。HTTP服务与后台线程共用已有Redis和配置，在同一进程内运行。
...
验证通过且交付完成后，立即删除本次批次临时目录。失败或降级时保留现场。每次任务结束后，对剩余Excel、JSON、CSV等文件执行容量清理；按修改时间从旧到新删除，直到同时满足100个文件和1 GiB上限。不跟随符号链接，不删除飞书备份或Redis状态。被清理的本地报告不能继续作为审阅证据。独立诊断命令指定的其他输出目录由使用者管理。
