...
7. 保存阶段结果并发送卡片。副本已生成但填充未完成时，卡片展示副本链接和具体原因。
...
`summary.history_status` 和 `summary.forecast_status` 分别展示阶段状态，`target_url` 指向保留的副本，`fill_report_path` 指向本次报告。启用阶段未完成时，整体返回 `failed`，但 `copied` 和目标链接仍如实保留。已完成历史填充的重复触发会直接跳过该阶段，保留运营后续修改。
