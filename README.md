...
两个开关独立。均关闭时只搬运；只开历史时填近30天；只开预测时读取数仓作为计算依据，补齐结构后仅填写公式预估数量；两项都开时同时填写当前30天、去年同期30天、去年对应后续周期及公式预估。全公司商品行保留空白，兜底范围待确认。新品预测未纳入通用公式，启用预测处理新品会降级交付原始表。未配置的开关默认关闭，示例配置为历史开启、预测关闭。支持 `true/false`、`1/0`、`yes/no`、`on/off`，大小写不敏感，非法值会阻止启动。开关控制 Webhook 和手动重新搬运流程；显式执行的历史检查、补列及填充命令按其命令参数运行。
...
线上公式预估填充、开关及副本验收见 [公式预估填充](docs/forecast-fill.md)。
